 ...
 */
	// public function render() {
		...
		// }
	// }
	// デバッグモード(0)で無ければ、標準のエラー画面ではなく、独自のエラー画面を出力。
    public function render() {
        if ($this->method && Configure::read('debug')) {
            call_user_func_array(array($this, $this->method), array($this->error));
        } else {
            ...
            $this->controller->layout = 'for_release';
            $this->controller->set(array(
                'code' => $code,
                'url' => h($url),
                'name' => $this->error->getMessage(),
                'error' => $this->error,
            ));
            try {
                $this->_outputMessage('for_release');
            } catch (Exception $e) {
                $this->_outputMessageSafe('for_release');
            }
        }
    }
